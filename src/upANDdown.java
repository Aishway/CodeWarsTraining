...
//For the first fixed tests the needed number of moves to get property (P) is given as a comment so that you can know if your process follows the rule.
    ...
    public static String arrange(String strng) {
        String[] stringToArray = strng.split(" ");

        for (int i = 0; i < stringToArray.length - 1; i++) {
            if (i % 2 == 0) {
                if (stringToArray[i].length() > stringToArray[i + 1].length()) {
                    String temporal = stringToArray[i];
                    stringToArray[i] = stringToArray[i + 1];
                    stringToArray[i + 1] = temporal; }
        ...
        for (int i = 0; i < stringToArray.length; i++) {
            if (i % 2 != 0) {
                stringToArray[i] = stringToArray[i].toUpperCase(Locale.ROOT);
